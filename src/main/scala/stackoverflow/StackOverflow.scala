...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    val rddQ = postings.filter(_.postingType == 1)
      .map(x => (x.id: QID, x))
    val rddA = postings.filter(x => x.postingType == 2)
      .map(x => (x.parentId.get: QID, x))
    val rddQA = rddQ.join(rddA)
    rddQA.groupByKey()
  }
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.map(x => (firstLangInTag(x._1.tags, langs).get*langSpread, x._2)).persist()
...
//    val newMeans = means.clone() // you need to compute newMeans
    ...
    val clustered = vectors.map(x => (findClosest(x, means), x))
    val clusterAvg = clustered.groupByKey().mapValues(averageVectors).collect().toMap
    val newMeans = means.indices.map(x => clusterAvg.getOrElse(x, means(x))).toArray
